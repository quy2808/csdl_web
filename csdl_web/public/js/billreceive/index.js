...
function config() {
	...
	$(".btn-edit").click(function (e) {
		...
		var nhanhang = $(this).data("nhanhang");
		...
		$("#EditUserModal input[name='nhanhang']").val(nhanhang);
	...
	$(".btn-delete").click(function (e) {
		...
		doc.text('METHOD:     ' + nhanhang, 20, 130)

		doc.setFontSize(25);
		doc.text('Receiver',220, 150)
		doc.setFontSize(20);
		doc.text('(signature and full name)',200, 160)
...
$("#job").on('change', function (e) {
	...
	if(selected != '-1') {
		$.ajax({ 
		  method: "POST",       
		  url: "index.php?page=admin&controller=billreceive&action=search",
		  data: {'type': selected}
		}).done(function( data ) { 
		  ...
		  result.map((data, i) => {
			  ...
			  "<tr class=\"text-center\">"+
				  ...
				  "<td>" + data.Hinhthuc_nhanhang + "</td>" +
				  "<td>" +
				  "<btn class='btn-edit btn btn-primary btn-xs' style='margin-right: 5px' data-nv='" + data.NV_taobienban + "' data-giaohang='" + data.qua_trinh_giao_hang + "' data-gia='" + data.Mucphi + "' data-gui='" + data.Nguoi_gui + "' data-nhan='" + data.Nguoi_nhan + "'data-hang='" + data.Thongtin_hang + "' data-ngay='" + data.Ngaygui + "'data-nhanhang='" + data.Hinhthuc_nhanhang + "'> <i class='fas fa-edit'></i></btn>" +
				  "<btn class='btn-delete btn btn-danger btn-xs' style='margin-right: 5px' data-nv='" + data.NV_taobienban + "' data-giaohang='" + data.qua_trinh_giao_hang + "' data-gia='" + data.Mucphi + "' data-gui='" + data.Nguoi_gui + "' data-nhan='" + data.Nguoi_nhan + "'data-hang='" + data.Thongtin_hang + "' data-ngay='" + data.Ngaygui + "'data-nhanhang='" + data.Hinhthuc_nhanhang + "'> <i class='fas fa-file'></i></btn>" +           
				  "</td>" +
